// 角色装备基础模型：集中管理部位定义、装备集合和预设 loadout 映射。
...
export type EquipSet = { [k in SlotKey]: string } & { consumables: string[]; name: string; ammoId: string; ammoRounds: number }
...
export function emptySet(name = ''): EquipSet {
  return { weaponId: '', armorId: '', chestRigId: '', backpackId: '', helmetId: '', headsetId: '', consumables: [], name, ammoId: '', ammoRounds: 0 }
}
...
export function fromLoadout(l: PlayerLoadout): EquipSet {
  return {
    ...
    helmetId: l.helmetId, headsetId: l.headsetId, consumables: [...l.consumables], name: '', ammoId: '', ammoRounds: 0,
  }
}
