...
func eventDefinitions() []models.EventDef {
	return []models.EventDef{
		...
		event("evt_route_blockade", "撤离路线封锁", "临时障碍和火力点截断了短路。", "evacuation", "encounter", "once_per_run",
			styled(checked("detour", []string{modeEvacuating}, "survival", 55, "", 0, "找到未被监视的穿行缺口。", "绕行失败，封锁守卫发现了目标。", effects(fx("evac_shortcut", "", 2), fx("time", "", 2)), effects(fx("encounter", "guard", 0))), []string{"balanced", "stealth"}, "bypass", 30),
		...
		event("evt_hidden_shortcut", "撤离捷径", "一条维护通路可能直达外围。", "evacuation", "", "once_per_run",
			checked("identify", []string{modeEvacuating}, "perception", 50, "", 0, "确认通路方向，缩短一段路线。", "入口已经堵死。", effects(fx("evac_shortcut", "", 2)), effects(fx("time", "", 2)))),
		...
		event("evt_tunnel_drain", "地下排水支路", "旧排水支路朝码头方向延伸。", "node", "", "once_per_run",
			checked("trace", []string{modeEvacuating}, "survival", 50, "", 0, "确认支路可用，缩短一段撤离路线。", "支路尽头已经坍塌。", effects(fx("evac_shortcut", "", 2)), effects(fx("time", "", 3)))),
