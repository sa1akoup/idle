...
func (r *Repository) queryItems(itemIDs []string) error {
	...
	if r.db.Migrator().HasTable(&models.KeyCaseDef{}) {
		if err := r.db.Where("id IN ?", itemIDs).Find(&keyCases).Error; err != nil {
			return fmt.Errorf("读取钥匙包商品: %w", err)
		}
	}
	for _, keyCase := range keyCases {
		if _, exists := r.items[keyCase.ID]; exists {
			continue
		}
		r.items[keyCase.ID] = Item{
	...
	if r.db.Migrator().HasTable(&models.SecureContainerDef{}) {
		if err := r.db.Where("id IN ?", itemIDs).Find(&secureContainers).Error; err != nil {
			return fmt.Errorf("读取安全箱商品: %w", err)
		}
	}
	for _, container := range secureContainers {
		if _, exists := r.items[container.ID]; exists {
			continue
		}
		r.items[container.ID] = Item{
