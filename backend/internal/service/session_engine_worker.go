...
func (s *SessionService) finishUnscheduledSession(sess *models.Session, state *engine.EngineState, snapshot engine.ScenarioSnapshot, now time.Time, reason string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if result := query.Updates(map[string]interface{}{
			...
			"pending_run_index": 0, "pending_run_result": "{}", "pending_run_hash": "", "state_json": string(encoded), "ammo_id": "", "ammo_rounds": 0,
		}); result.Error != nil {
			return result.Error
		} else if result.RowsAffected != 1 {
			return ErrSessionLeaseLost
		}
		return appendSessionEventTx(tx, s.userID, sess.ID, sess.PendingRunIndex, sessionEventSessionFinished, sess.ElapsedSec, now, "", "", map[string]interface{}{"status": "success", "reason": reason})
	})
}
