...
> 账号登录 · 用户数据隔离 · 纯引擎重放 · 实时探索 · 离线补算 · 分级弹药战斗 · 技能/声望闭环

方向文档：`搜打撤放置游戏V0.3更新方向.md`；改造方案：`docs/v0.3-改造方案.md`。  
下一版本：`搜打撤放置游戏V0.4更新方向.md`、`docs/v0.4-改造方案.md`（黑市商人、第二张图与 Boss；不做 Scav）。
...
- [x] V0.3-A：角色白板主属性 25；撤离成功成长技能；会话成功与出售提升商人好感；机械商人 LL1 开放；情报中心加成写入快照
- [x] V0.3-B：商人合同在探索页接取/上交；上交只扣撤离带出（raidExtract）物品，商店购买无效，撤离成功不自动交付
- [x] V0.4：黑市商人 LL1（跨类低价收购，6 小时加权货架，售价翻倍）；第二张图林缘厂房，指定节点低概率刷 Boss
